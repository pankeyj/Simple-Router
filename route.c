	...
	****************************************/
		if(strncmp(&(tmp->ifa_name[0]),"lo",2))
		{
			...
			if(tmp->ifa_addr->sa_family==AF_PACKET){
				...
				int i;
				int len = 0;
				// Print the mac address of this interface for debugging
				for(i = 0; i < 6; i++)
					printf("%02X%s",s->sll_addr[i],i < 5 ? ":":"\n");
	
				// Find the mac address of the current interface
				// Save this mac address to the global variable
				// src_mac[id] where id is the interface index
				for(i = 0; i < 6; i++)
				...
				// Print the ip address of this interface for debugging
	...
	freeifaddrs(ifaddr);
	// Don't return so that program continues to run
...
{	
	...
	while(1)
	{
		...
		// These pieces remain from the beginning of the project
		int offset = 0;	
...
void buildArpResponse(int id)
{
	...
	// Copy the appropriate ip and mac addresses
...
unsigned char* sendArpRequest(unsigned char nextHop[4], int id)
{
	...
	// Copy values into the ip and mac address
	// fields. Mac destination is broadcast for
	// the arp request
 ...
 * This function is called to receive an arp response
 * The purpose of this function is to fill in the global
 * variable for dest_mac[id]
 **************************************************/
void recvArpResponse(char buf[1500], int id)
{
        int result, offset,i;
...
/*****************************************************
 * @param buf - The buffer received
 * @param id - Id of the interface that received message
 * This function is called when an icmp request is 
 * received and the target is the current node.
 * This buffer includes an ethernet header an ip header
 * an icmp header and the original data received.
 ****************************************************/
...
//	icmphdr->icmp_chksum = ip_checksum((const void*)tmpbuf,sizeof(tmpbuf));
	...
	// Fill in the check sum field of the icmp header
